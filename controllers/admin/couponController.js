...
const addCoupon = async (req, res) => {
  const couponValidationRules = [
    body('name')
      ...
      .matches(/^[A-Z0-9]{4,10}$/)
      .withMessage('Coupon code must be 4-10 uppercase alphanumeric characters'),
    body('discountType')
      .notEmpty()
    ...
    body('offerPrice')
      ...
      .isFloat({ min: 0.01 })
      .withMessage('Offer price must be at least 0.01 for fixed amount discounts'),
    ...
    body('islist')
      .isBoolean()
      .withMessage('islist must be a boolean'),
  ];

  try {
    console.log('Request body:', req.body); // Debug log
    await Promise.all(couponValidationRules.map(validation => validation.run(req)));
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({ errors: errors.array() });
    }

    const { name, discountType, offerPrice, offerPercentage, minimumPrice, maxUsage, expireOn, islist } = req.body;

    const coupon = new Coupon({
      name: name.toUpperCase(),
      discountType,
      offerPrice: discountType === 'price' ? parseFloat(offerPrice) : null,
      offerPercentage: discountType === 'percentage' ? parseFloat(offerPercentage) : null,
    ...
    if (error.code === 11000) {
      return res.status(400).json({ errors: [{ msg: 'Coupon code already exists' }] });
    }
...
const getCoupons = async (req, res) => {
  try {
    const coupons = await Coupon.find().select('name discountType offerPrice offerPercentage minimumPrice maxUsage expireOn islist usedBy')
...
const updateCoupon = async (req, res) => {
  ...
  try {
    ...
    const { name } = req.params;
    console.log('Coupon name:', name);
    const { discountType, offerPrice, offerPercentage, minimumPrice, maxUsage, expireOn } = req.body;
...
const getCouponsUsers = async (req, res) => {
 ...
 const coupon = await Coupon.findOne({ name: name.toUpperCase() })
  ...
  console.log(coupon)
    if (!coupon) {
      console.log('Coupon not found for name:', name);
      return res.status(404).json({ errors: [{ msg: 'Coupon not found' }] });
    }
